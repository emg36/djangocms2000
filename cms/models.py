...
class Block(ContentModel):
    ...
    language = models.CharField(
        max_length=10, choices=cms_settings.LANGUAGES, default=settings.LANGUAGE_CODE
    )
    format = models.CharField(
        max_length=10, choices=FORMAT_CHOICES, default=FORMAT_PLAIN
    )
...
UNSUPPORTED_ERROR = "Unsupported file type. Supported types: %s" % ", ".join(
    VALID_TYPES
)
...
class Video(ContentModel):
    source = models.FileField(
        upload_to=cms_settings.UPLOAD_PATH, validators=[validate_video_type], blank=True
    )
...
class _CMSAbstractBaseModel(models.Model):
    ...
    def get_blocks(self):
        return Block.objects.filter(content_type=key_from_obj(self), object_id=self.id)

    def get_images(self):
        return Image.objects.filter(content_type=key_from_obj(self), object_id=self.id)
...
class Page(_CMSAbstractBaseModel):
    ...
    def __str__(self):
        block = self.get_blocks().exclude(content="").filter(label="title").first()
        return block.content if block else ("Page: %s" % self.url)


# @python_2_unicode_compatible
class PageSite(models.Model):
    page = models.ForeignKey(Page, related_name="sites", on_delete=models.CASCADE)
    ...
    if isinstance(kwargs["instance"], CMSBaseModel):
        ...
        for label in kwargs["instance"].IMAGE_LABELS:
            Image.objects.get_or_create(
                label=label, content_type=ctype, object_id=kwargs["instance"].id
            )
